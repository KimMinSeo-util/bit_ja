...
select * from users where id='java01' and password='1234'; 		--id와 password값이 일치하면 로그인 성공
select nvl(id,'실패') from users where id='java01' and password='java01'; 

alter table board modify(title varchar2(40));
alter table board modify(content varchar2(100));

/* 게시판 글등록 */
create sequence seq; --게시물 번호
 
insert into board 
values (seq.nextval, 'java의 정석','java for while loop', '19/05/05', default, 'java01');

insert into board 
values (seq.nextval, 'sql 더쉽게, 더 깊게', 'create grant select','19/02/24', default, 'JAVA');
